...
The lean profile was then run rather than assumed, because 4 GiB for the manager is below Wazuh's own recommendation for an all-in-one deployment and a number that low is worth measuring rather than asserting. With the manager tuned, the endpoint enrolled and both guests idle, the manager used 2,320 MB of the 5,555 MB the balloon had given it, of which wazuh-indexer was 1,407 MB, wazuh-manager 247 MB, wazuh-dashboard 162 MB and filebeat 10 MB; the endpoint used 391 MB of 824 MB, with the five agent daemons summing to 62 MB.

Alert latency was measured by a watcher on the manager so that the polling stayed local and only the findings crossed the network. Across eleven samples from three S1 runs, a rule's own `@timestamp`, which analysisd assigns when it decides an event matched, lands 0.9 to 4.9 seconds after the action for the single-event rules and 17.4 to 22.9 seconds after the start of the scenario for the level 10 correlation, which is the time the scenario itself spends generating six failures. Indexing adds 3.4 to 7.5 seconds on top, median 5.1, which is filebeat's batching plus the indexer's refresh. That second number is the one a person watching the dashboard experiences.

The load ceiling is not the manager. Three minutes of real SSH logins to the endpoint, 4,388 of them at 24.4 a second, put 17,100 alerts into one hour against 1,046 in the hour before, a sustained rate of about 73 alerts a second where the lab idle produces well under one. All four manager services stayed up, guest memory rose 58 MB, the indexer's resident size did not move and analysisd's rose 21 MB and held. The whole pipeline's lag, measured as the age of the newest alert the indexer held, stayed between 4.0 and 7.8 seconds throughout and did not grow, which is the same figure as at idle. What did strain was the endpoint: rule 202, `Agent event queue is 90% full`, fired once on the 1 GiB guest, and auth.log rotated under the volume. So on the lean profile the first thing to give is the agent's queue on the endpoint, not the manager, and the published 8 GB host floor stands.

One thing that run also settled: the alert fetch ceiling of 5,000 documents is reachable. The fifteen minute window held 16,631. The helper set `truncated`, the dashboard read it and said so, and the panels are drawn from aggregations that count every matching document whatever the fetch returns, so nothing was lost silently. The comment that had estimated a few hundred alerts per busy window was wrong and now carries the measurement instead.
