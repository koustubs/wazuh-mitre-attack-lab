...
install -o root -g wazuh -m 640 "$project/04-implementation/manager/lab_rules.xml" \
...
python3 "$project/04-implementation/tests/test_rules.py" \
    --output "$project/04-implementation/evidence/rule-checks.json" \
